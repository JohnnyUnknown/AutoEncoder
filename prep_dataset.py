...
output_dir = Path(r"C:\My\Projects\images\main\Data_img\Dataset")

# Размер для изменения
target_size = (360, 360)  # (ширина, высота)
    ...
    for img_path in paths:
        try:
            with Image.open(img_path) as img:
                ...
                # Сохраняем в нужную папку с тем же именем файла
...
def process_and_save_train(paths):
    ...
    for img_path in paths:
        ...
        try:
            with Image.open(img_path) as img:
                save_path = (output_dir / f'train{cnt // 500}') / img_path.name
                img.save(save_path)
                # print(f"Сохранено: {save_path}")
        except Exception as e:
            print(f"Ошибка обработки {img_path}: {e}")
