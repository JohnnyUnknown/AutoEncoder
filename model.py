...
# Конфигурация
IMAGE_DIR = Path(r"C:\My\Projects\images\main\Data_img\Dataset2")
TARGET_HEIGHT = 360
TARGET_WIDTH = 360
...
# 
# # Encoder
# x = Conv2D(64, kernel_size=3, activation='relu', padding='same', kernel_regularizer=regularizers.l1(1e-4))(inputs)
# x = Conv2D(64, kernel_size=3, activation='relu', padding='same', kernel_regularizer=regularizers.l1(1e-4))(inputs)
# x = MaxPooling2D(pool_size=2, padding='same')(x)  
# x = Conv2D(128, kernel_size=3, activation='relu', padding='same', kernel_regularizer=regularizers.l1(1e-4))(x)
# x = Conv2D(128, kernel_size=3, activation='relu', padding='same', kernel_regularizer=regularizers.l1(1e-4))(x)
# x = MaxPooling2D(pool_size=2, padding='same')(x)  
# x = Conv2D(256, kernel_size=3, activation='relu', padding='same', kernel_regularizer=regularizers.l1(1e-4))(x)
# x = Conv2D(256, kernel_size=3, activation='relu', padding='same', kernel_regularizer=regularizers.l1(1e-4))(x)
# # x = Conv2D(256, kernel_size=3, activation='relu', padding='same')(x)
# x = MaxPooling2D(pool_size=2, padding='same')(x)  
# x = Conv2D(512, kernel_size=3, activation='relu', padding='same', kernel_regularizer=regularizers.l1(1e-4))(x)
# x = Conv2D(512, kernel_size=3, activation='relu', padding='same', kernel_regularizer=regularizers.l1(1e-4))(x)
...
# x = Conv2D(128, kernel_size=3, activation='relu', padding='same', kernel_regularizer=regularizers.l1(1e-4))(x)
# x = UpSampling2D(size=2)(x)    
# x = Conv2D(64, kernel_size=3, activation='relu', padding='same', kernel_regularizer=regularizers.l1(1e-4))(x)
# x = Conv2D(64, kernel_size=3, activation='relu', padding='same', kernel_regularizer=regularizers.l1(1e-4))(x)
# x = UpSampling2D(size=2)(x) 
...
for i in range(1, 9):
    if i > 0: 
        autoencoder_model = load_model(Path(path[0] + f"/my_autoencoder{i-1}.keras"))
        autoencoder_model.summary()
    ...
    file_model = Path(path[0] + f"/my_autoencoder{i}.keras")
